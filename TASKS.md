...
- [x] Retry the same model without `reasoning_effort` when the judging proxy rejects it with
  HTTP 400, then disable that optional field for the remaining batch
- [x] Reuse one Fireworks client for all tasks so proxy capability state persists run-wide
...
- [x] Reproduce the v5 proxy failure with a rejecting mock and verify v6's bare-request fallback
- [x] Run v6 against a real 19-task container batch: 19 nonempty schema-valid answers, 1,256 tokens
...
- [x] V5 accuracy-first candidate `budgetbrain-track1:champion-v5-accuracy` is a single
  `linux/amd64` manifest, `45,530,242` bytes
- [x] V6 proxy-compatible candidate `budgetbrain-track1:champion-v6-proxyfix` is a single
  `linux/amd64` manifest, `45,530,446` bytes
...
- [x] Push and anonymously verify the immutable v5 accuracy-first tag
- [x] Record the v5 digest:
  `sha256:ae93738ccde9c56c0f20ff2a9e13ea29e2917907727d6406dcd00b45c937bc9c`
- [x] Record the v5 evaluator result: `63.2%` (`12/19`)
- [x] Push and anonymously verify the immutable v6 proxy-compatible tag
- [x] Record the v6 digest:
